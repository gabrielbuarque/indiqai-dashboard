...
class DashboardIndiqaiSupabaseUser extends BaseAuthUser {
  DashboardIndiqaiSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> dashboardIndiqaiSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DashboardIndiqaiSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
